...
Ce document est un aide mémoire personnel que je mets à disposition de tous sous licence *MIT*. Faites-en ce que vous voulez.
...
Python n’est certainement pas le langage le plus performant qui soit pour la création de jeux vidéos mais apprendre la programmation de jeux demande de nombreuses compétences et si vous débutez, il est préférable de vous simplifier les choses autant que possible.
L’interpréteur Python est libre, gratuit et s’installe facilement. Installer et utiliser des bibliothèques supplémentaires (modules) est très simple. Apprendre les bases de la programmation avec Python est plus facile qu’avec d’autres langages et vous oblige à prendre de bonnes habitudes. De plus, la bibliothèque de jeux cocos2d existe pour de nombreux autres langages de programmation. Lorsque vous vous sentirez prêt, vous pourrez donc changer de langage tout en continuant à utiliser une bibliothèque que vous connaissez. Ainsi, vous n’aurez pas perdu votre temps. Enfin, apprendre Python pourra vous servir pour d’autres choses que les jeux vidéos et vos connaissances seront applicables dans n’importe quel autre domaine de programmation.
...
Et cliquez sur la section Downloads pour télécharger la version correspondant à votre système. Ensuite installez Python. Sous Windows, pensez à cocher l’option ajouter au chemin. Python possède deux versions. La version 2 est l'ancienne version qui ne sera plus mise à jour en 2020. Préférez la version 3.

Remarque : Sur Windows, pensez à cocher la case `Add Python 3.x to PATH` lors de l'installation. Sur Mac OS X, Python est déjà installé mais c'est la version 2. Vous pouvez installer la version 3 sans risque en utilisant la commande suivante :

`sudo apt-get install python3`

Pour les autres implémentations, consultez leurs sites respectifs mais sachez que certaines bibliothèques pourront ne pas fonctionner. Vous devrez donc expérimenter par vous-même. Vous voilà averti !
...
PyCharm est assez réputé.

https://www.jetbrains.com/pycharm/

Téléchargez la version Community qui est gratuite.

#### Geany

Geany est un éditeur très léger et disponible sur de nombreuses plateformes (y compris le Raspberry Pi).
...
Pour voir quelle version de Python est installée sur votre machine, saisissez la commande suivante :
...
Vous devriez avoir les signes suivants apparaître pour vous indiquer que vous êtes passé en mode interactif :
...
which python affiche le chemin du projet courant et l'environnement virtuel en cours d'utilisation.
...
La fonction print affiche dans le terminal le ou les paramètres passés en arguments séparés par un espace puis effectue une retour à la ligne. Vous pouvez également passer des littéraux de type chaînes de caractères en argument :
...
Le type booléen correspond à des valeurs logiques qui peuvent prendre soit la valeur True (vrai), soit la valeur False (faux).
...
En Python, il n'existe pas de type caractère. Utilisez à la place les chaînes de caractères. Elles font partie des séquences (voir plus loin).
...
Renvoie la chaîne en caractères minuscules :

chaine.lower()

Renvoie la chaîne en caractères majuscules :

chaine.upper()

Compte le nombre d'occurrence d'une sous-chaîne :

chaine.count(sous_chaine)

Renvoie l'index d'une sous-chaîne :

chaine.find(sous_chaine)

Remarque : Renvoie -1 si la sous-chaîne n'existe pas dans la chaîne.

Renvoie une chaîne avec une sous-chaîne remplacée par une autre :

chaine.replace(sous_chaine_à_remplacer, sous_chaine_de_remplacement)

Renvoie une copie de la chaîne avec des espaces supplémentaires si nécessaires pour centrer la chaîne d'origine sur un nombre de caractères spécifié en argument :
...
L'opérateur * duplique la chaîne un certain nombre de fois :

chaine * nombre

#### Longueur d'une chaîne

Pour déterminer la longueur (le nombre de caractères) d'une chaîne, utilisez la fonction len :
...
Enfin, vous pouvez attribuer aux arguments des clés que vous pouvez placer entre les accolades :
...
La méthode split découpe une chaîne en utilisant un séparateur passé en argument :
...
Il existe deux types de base pour représenter des nombres en Python. Les entiers relatifs int et les nombres à virgule flottante float.
...
continue passe à l'itération suivante sans exécuter les instructions qui suivent dans la boucle.
pass permet d'insérer une boucle sans instruction.
...
Mot clé pass
Utilisez le mot clé pass pour indiquer qu'un bloc d'instructions n'est pas encore nécessaire. Par exemple, après une instruction if, while, for ou dans le corps d'une fonction.

## Assertions

```assert conditions_à_évaluer```

Si la condition est fausse, une exception de type AssertionError est levée.

## Lever une exception

Pour déclencher (lever) une exception vous-même, utilisez le mot clé raise suivi d'un appel au constructeur du type de l'exception à lever. Les constructeurs des exceptions prennent en général en argument une chaîne de caractères décrivant l'erreur.
...
Accédez aux éléments via un index commençant à 0 :

liste[1]

Vous pouvez utiliser un index négatif pour partir de la fin.
L'index -1 est équivalent à l'index len(liste) - 1
Slicing s'applique également à ce type.
La méthode append permet d'ajouter un élément à la fin de la liste.

liste.append(élément)

La méthode insert permet d'ajouter un élément à un index particulier.

liste.insert(index, élément)

L'élément situé à l'index et les suivants sont décalés d'une position et leur index respectifs sont incrémentés d'un.
...
Même si ce n'est pas très élégant, vous pouvez également utiliser la fonction del pour supprimer un élément :
...
Si vous passez un index, c'est l'élément situé à cette position qui sera supprimé de la liste et renvoyé :
...
La fonction sorted prend une liste en argument et retourne une copie triée :
...
Les tuples sont immutables. Cela signifie qu'ils ne peuvent être modifiés une fois créés. Considérez-les comme des listes constantes.
...
Pour créer un set, utilisez les accolades.
...
Utilisez la fonction set pour créer une liste vide :
...
La fonction len renvoie le nombre d'éléments du dictionnaire :
...
Remarque : Par convention, toutes les fonctions en Python ont une chaîne de caractères pour décrire ce que fait la fonction, ses paramètres et, le cas échéant, son type de retour.
...
Remarque : Les paramètres nommés doivent apparaître après les arguments positionnels.
...
Combinez les deux dans cet ordre :
...
Remarque : Vous n'avez pas besoin d'utiliser l'extension .py pour importer un fichier.
...
Pour importer directement le nom de l'objet ou de la fonction d'un module sans le faire précéder du nom du module, utilisez la syntaxe suivante :
...
Importer tous les objets et fonctions d'un fichier est une mauvaise pratique mais vous pouvez le faire avec la ligne suivante :
...
try:
    ...
    instructions_à_exécuter_en_cas_d'erreur
...
Attention ! Si le fichier n'existe pas, seul le mode='w' crée un un nouveau fichier. Les autres modes provoquent une erreur.
...
Pour éviter l'oubli de cette méthode, utilisez la syntaxe suivante :

with open('nom_de_fichier.txt') as fichier:
    # instructions

A la fin de ce bloc, le fichier sera fermé automatiquement.
